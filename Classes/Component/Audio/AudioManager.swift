...
class AudioManager: NSObject {
    ...
    func audioDir() -> NSURL{
        ...
        let documentsFolderUrl = fileManager.URLForDirectory(.DocumentDirectory, inDomain: .UserDomainMask,
            ...
            create: false,
            error: nil)
        let userFolderUrl = documentsFolderUrl!.URLByAppendingPathComponent("\(User().uid)")
        if !fileManager.fileExistsAtPath(userFolderUrl.absoluteString!){
    ...
    func saveAudio(data: NSData, name: NSString) -> Bool{
        if(name.length > 0){
            ...
            data.writeToFile(path.absoluteString!, options: NSDataWritingOptions.DataWritingFileProtectionComplete, error: &error)
            if (error == nil){
                return true
            }
        }
