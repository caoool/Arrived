...
extension UIViewController {
    ...
    // my customization
    func interestedNotice(text: String, autoClear: Bool) {
        SwiftNotice.showInterestedNotice(text, autoClear: autoClear)
    }
}
...
class SwiftNotice: NSObject {
    ...
    static func showText(text: String) {
        ...
        label.font = UIFont(name: "American Typerwriter", size: 12)
        label.textAlignment = NSTextAlignment.Center
        label.textColor = UIColor.whiteColor()
        label.sizeToFit()
    ...
    static func showInterestedNotice(text: String, autoClear: Bool) {
        let frame = CGRectMake(0, 0, 90, 90)
        let window = UIWindow(frame: frame)
        let mainView = UIView(frame: frame)
        mainView.layer.cornerRadius = 15
        mainView.layer.borderColor = UIColor.lightGrayColor().CGColor
        mainView.layer.borderWidth = 0.35
        ...
        */
        ...
        var image = UIImage(named: "interested icon")
        let iconView = UIImageView(image: image)
        iconView.frame = CGRectMake(20, 15, 50, 50)
        mainView.addSubview(iconView)
        
        let label = UILabel(frame: CGRectMake(0, 60, 90, 20))
